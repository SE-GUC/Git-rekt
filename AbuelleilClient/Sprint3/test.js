...
test(`Admin name should be youssef321`, async () => {
    ...
    const createdAdmin = await axios.put(`http://localhost:3000/api/admin/updateAdmin/${xid}`,newAdmin)
    expect(createdAdmin.status).toEqual(200)
    expect(createdAdmin.data.data.name).toEqual('youssef321')
});
test(`Admin Get`, async () => {
    const user =  await axios.get(`http://localhost:3000/api/admin/${xid}`)
    expect(user.status).toEqual(200)
    expect(user.data.name).toEqual('youssef321')
});
test(`Admin should be deleted`, async () => {
    const deleted = await axios.delete(`http://localhost:3000/api/admin/${xid}`)
    expect(deleted.data.data.name).toEqual('youssef321')
