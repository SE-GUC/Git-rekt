...
class UserViewAllAvailableTasks extends Component {
    ...
    async setAvailableTasks () {
        ...
        try {
            ...
            this.setState({availableTasks: finalTasks, isLoading:false})  
    ...
    async componentDidMount() {
        this.setState({isLoading: true})
        try{
            const user = await axios.get("http://localhost:3001/api/user/5cb31b9c55ba5b0b5cb9fc92");
    ...
    render() {
        const {id, certifications, availableTasks, isLoading, error} = this.state;
        if (error) {
        ...
        return(
            <div>
                <ViewTasks Tasks = {availableTasks} UserId = {id} />
            </div>
        );
    }
}
